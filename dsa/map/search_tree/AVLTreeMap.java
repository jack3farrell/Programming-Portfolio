...
public class AVLTreeMap<K extends Comparable<K>, V> extends BinarySearchTreeMap<K, V> {
	...
	private void rebalance(Position<Entry<K, V>> p) {
		...
		do {
			oldHeight = getProperty(pos);
			if (!isBalanced(pos)) {
				// Find child with taller height
				Position<Entry<K, V>> child = tallerChild(pos);
				// Find grandChild with taller height
				Position<Entry<K, V>> grandChild = tallerChild(child);
				// Tri-node restructure and save root of restructured subtree as p
				pos = restructure(grandChild);
				recomputeHeight(left(pos));
				recomputeHeight(right(pos));
			}
			recomputeHeight(pos);
			newHeight = getProperty(pos);
			pos = parent(pos);
		} while (oldHeight != newHeight && pos != null);
	}
