...
//        Entry<K, V> temp = list.removeFirst();
//        upHeap(list.size() - 1);
//        return temp;
	...
	protected void upHeap(int index) {
		// For check style error
		int i = index;
		while (i > 0) {
			int parent = parent(i);
			if (compare(list.get(i).getKey(), list.get(parent).getKey()) >= 0) {
				break;
			} else {
				swap(i, parent);
				i = parent;
	...
	protected void downHeap(int index) {
		...
		while (hasLeft(i)) {
			...
			if (compare(list.get(smallestChildIndex).getKey(), list.get(i).getKey()) >= 0) {
				break;
			}
